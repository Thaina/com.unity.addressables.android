    ...
    public class BuildScriptPackedModeODR : BuildScriptBase
    {
        ...
        private Dictionary<string, List<ContentCatalogDataEntry>> GetPrimaryKeyToDependerLocations(List<ContentCatalogDataEntry> locations)
        {
            ...
            foreach (ContentCatalogDataEntry location in locations)
            {
                ...
                {
                    if (location.Dependencies[i] is not string dependencyKey || string.IsNullOrEmpty(dependencyKey))
                        continue;
        ...
        internal bool CreateCatalogFiles(byte[] data, AddressablesDataBuilderInput builderInput, AddressableAssetsBuildContext aaContext, string catalogHash = null)
        {
            ...
            WriteFile(m_CatalogBuildPath.Replace(".bin", ".hash"), HashingMethods.Calculate(data).ToString(), builderInput.Registry);
            ...
            ProviderLoadRequestOptions catalogLoadOptions, string contentHash)
        {
            ...
            else
            {
                ...
                dependencyHashes[(int)ContentCatalogProvider.DependencyHashIndex.Local] = ResourceManagerRuntimeData.kCatalogAddress + "LocalHash";
                ...
                var localCatalogLoadPath = "{UnityEngine.AddressableAssets.Addressables.RuntimePath}/catalog.hash";
...
#if UNITY_IOS
                var odrSchema = assetGroup.GetSchema<PlayAssetDeliverySchema>();
                if(odrSchema != null)
                {
                    if(string.IsNullOrEmpty(path))
                        Debug.LogWarning($"ODR Schema build path value is null");

                    targetPath = Path.Combine(path,Path.GetFileName(targetPath));
                    Debug.Log("targetPath : " + targetPath + "\n" + Path.GetFullPath(targetPath));
                    AddBundleODR(outputBundleNames[i],targetPath,assetGroup.Name,odrSchema.AssetPackDeliveryType,i);
                }
#endif
        ...
        public static IReadOnlyCollection<BundleInfoODR.Bundle>? Bundles => m_Resources?.m_bundles?.AsReadOnly();

        static string ODRSettingsPath => Path.Combine(Addressables.BuildPath,"settings_odr.json");
        public static void CleanBundleInfoODR()
        {
            DeleteFile(ODRSettingsPath);
            m_Resources = new BundleInfoODR();
        }
        public static void AddBundleODR(string odr_name, string odr_path, string odr_tag, DeliveryType odr_category, int odr_order)
        {
            m_Resources.m_bundles.Add(new BundleInfoODR.Bundle() { name = odr_name, path = Path.GetFullPath(odr_path), tag = odr_tag.Replace(" ","_").ToLowerInvariant(), category = odr_category, order = odr_order });
        }

        public static void PreserveODRSettings(FileRegistry registry)
        {
            WriteFile(ODRSettingsPath, JsonUtility.ToJson(m_Resources), registry);
        }

#if UNITY_IOS
        public static IReadOnlyCollection<BundleInfoODR.Bundle> CollectODR()
        {
            if(!(m_Resources?.m_bundles?.Count > 0))
            {
                // No info in memory, fetch from disk if there is data there
                var odrSettingsPath = ODRSettingsPath;
            ...
            return m_Resources.m_bundles.AsReadOnly();
        }

        public static IEnumerable<(DeliveryType,IEnumerable<string>)> CollectPrefetchCategories()
        {
            return CollectODR().GroupBy((bundle) => bundle.category,(key,values) => {
                return (key, values.OrderBy(b => b.order).Select((b) => b.tag));
            });
        }
        ...
        public class PostProcessODRBundles : IPostprocessBuildWithReport
        {
            ...
            internal const string InitialInstallTagsKey = "ON_DEMAND_RESOURCES_INITIAL_INSTALL_TAGS";
            internal const string PrefetchedTagOrderKey = "ON_DEMAND_RESOURCES_PREFETCH_ORDER";
            ...
            public void OnPostprocessBuild(BuildReport report)
            {
                ...
                pbxProject.SetBuildProperty(mainTargetGuid,"EMBED_ASSET_PACKS_IN_PRODUCT_BUNDLE","YES");

                // Set Prefetch Categories
                foreach(var group in CollectODR().GroupBy((bundle) => bundle.category))
                {
                    foreach(var bundle in group)
                    {
                        var realPaths = Directory.EnumerateFiles(Path.Combine(pathToBuiltProject,"Data"),bundle.name,SearchOption.AllDirectories).ToArray();
                        var (filePath, fileGuid) = realPaths.Select((realPath) => (realPath, guid: pbxProject.FindFileGuidByRealPath(realPath))).FirstOrDefault((pair) => !string.IsNullOrEmpty(pair.guid));
                        filePath = filePath ?? realPaths.FirstOrDefault();
                        if(!string.IsNullOrEmpty(filePath) && filePath.Contains("Data/Raw/") && File.Exists(filePath))
                            File.Delete(filePath);
                        if(!string.IsNullOrEmpty(fileGuid))
                            pbxProject.RemoveFileFromBuild(mainTargetGuid,fileGuid);

                        fileGuid = pbxProject.AddFile(bundle.path,"UnityData.xcassets/" + bundle.name,PBXSourceTree.Source);
                        pbxProject.AddFileToBuild(mainTargetGuid,fileGuid);
                        pbxProject.AddAssetTagForFile(mainTargetGuid,fileGuid,bundle.tag);
                    }

                    string key = group.Key switch {
                        DeliveryType.InstallTime => InitialInstallTagsKey,
                        DeliveryType.FastFollow => PrefetchedTagOrderKey,
                        _ => string.Empty,
                    };

                    if(!string.IsNullOrEmpty(key))
                    {
                        foreach(string tag in group.Select((bundle) => bundle.tag).Distinct())
                            pbxProject.AddBuildProperty(mainTargetGuid,key,tag);
                    }
                }
